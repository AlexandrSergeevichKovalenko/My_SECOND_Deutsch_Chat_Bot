        ...
        try:
            ...
            if was_in_mistakes:
                if score >= 85:
                    ...
                    cursor.execute("""
                        SELECT MAX(attempt) FROM detailed_mistakes_deepseek
                        WHERE sentence_id = %s AND user_id = %s;
                    """, (id_for_mistake_table, user_id))
                    result = cursor.fetchone()
...
def main():
    ...
    scheduler.add_job(lambda: run_async_job(get_yesterdays_mistakes_for_audio_message, CallbackContext(application=application)), "cron", hour=05, minute=15)
